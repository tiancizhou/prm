...
SELECT department_name, NULL, 0, 'ACTIVE', 0, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
FROM (
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_sys_department_name_parent_deleted
    ON sys_department (name, COALESCE(parent_id, 0), deleted);
